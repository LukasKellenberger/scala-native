...
case class MultirunSuccess(override val name: String, times: Seq[Long]) extends MultirunResult(name)
case class MultirunFail(override val name: String) extends MultirunResult(name)
case class MultirunDisabled(override val name: String) extends MultirunResult(name)
  ...
  def apply(results: Seq[BenchmarkResult]): MultirunResult = {
    ...
    if(results.exists(_.isInstanceOf[BenchmarkDisabled]))
      ...
      if(results.exists(_.isInstanceOf[BenchmarkFailed]))
        ...
        MultirunSuccess(name, results.flatMap{ case BenchmarkCompleted(_, times) => times })
      }
    }

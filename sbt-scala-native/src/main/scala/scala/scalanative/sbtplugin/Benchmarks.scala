...
object Benchmarks {
  def showResults(): Unit = {
	...
	line.split(":").toList match {
      ...
      if (rows.exists(_.isInstanceOf[FailedRow])) {
        ...
        val (iters, times) = rows.foldLeft((0, List[Long]())) {
          case ((totalIters, totalTimes), SuccessfulRow(_, i, t)) =>
		(totalIters + i, t.toList ::: totalTimes)
          case (acc, _) => acc
        }
    ...
    println(TextFormat.show(res.sortBy {
	case _ : CompletedBenchmark => 0
	case _ => 1
}))
